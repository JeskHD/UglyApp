                        ...
                        <article>
                            <div>
                                ...
                                <div class="form-container">
                                    ...
                                    {% if download_link %}
                                        <p>Download complete: <a href="{{ download_link }}">{{ download_link }}</a></p>
                                    {% endif %}
                                </div>
    ...
    try:
        ...
        if "twitter.com/i/spaces" in url or "x.com/i/spaces" in url:
            ...
            if process.returncode == 0:
                ...
                if os.path.exists(latest_file):
                    ...
                    session['download_link'] = url_for('upload', filename=os.path.basename(latest_file))
                    return redirect(url_for('index'))
                else:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if os.path.exists(file_path):
                end_time = time.time()  # End time for download
                time_taken = end_time - start_time  # Calculate total time taken
                session['download_link'] = url_for('upload', filename=os.path.basename(file_path))
                return redirect(url_for('index'))
            else:
                flash("File not found after download.")
                return redirect(url_for('index'))
